...
export function SectionTitle({eyebrow,title,copy}:{eyebrow:string;title:string;copy?:string}){return <header className="section-head"><Label>{eyebrow}</Label><h2>{title.split("\\n").map((line,i,lines)=><span key={`${line}-${i}`}>{line}{i<lines.length-1&&<br/>}</span>)}</h2>{copy&&<p className="lede">{copy}</p>}</header>}
